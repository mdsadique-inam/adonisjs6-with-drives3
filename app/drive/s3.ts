...
export class S3Driver {
   ...
   * and again
   */
  #cachedBucketUrl: string | undefined
  ...
  constructor(
    ...
    this.#cachedBucketUrl = undefined
  }

  /**
   * Transforms the write options to S3 properties
   */
  #transformWriteOptions(options?: WriteOptions) {
    const {
      visibility,
      contentType,
      contentDisposition,
   ...
   */
  #transformContentHeaders(options?: ContentHeaders) {
  ...
  async get(location: string): Promise<Buffer> {
    return getStreamAsBuffer(await this.getStream(location))
  }
   ...
   * Not supported
   */
  async getVisibility(location: string): Promise<Visibility> {
    try {
  ...
  async getSignedUrl(
    ...
    try {
      return await getSignedUrl(
        ...
        {
          expiresIn: string.milliseconds.parse(options?.expiresIn || '15min') / 1000,
        }
      )
    } catch (error) {
   ...
   */
  async put(location: string, contents: Buffer | string, options?: WriteOptions): Promise<void> {
    try {
      await this.adapter.send(
   ...
   * Not supported
   */
  async setVisibility(location: string, visibility: Visibility): Promise<void> {
    try {
      await this.adapter.send(
